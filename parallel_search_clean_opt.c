 ...
 *             Ускорение без изменения основной логики (поиск по чанкам)
 *
 * Оптимизации:
 *   1. Локальные счетчики в потоках (без мьютекса во время поиска)
 *   2. Выравнивание чанков по границам кэш-линий
 *   3. Оптимизированная функция поиска (быстрый отсев по первому байту)
 *   4. Уменьшение накладных расходов на создание потоков
 *   5. Предварительная проверка первого байта паттерна
 *   6. Избегание лишних проверок границ
 *
 * Компиляция:
